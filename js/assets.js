/* ============================================================
   ...
   ⚠️ 路径约定（与项目硬约束一致）：
     - 使用「相对站点根」的路径（如 'assets/titles/xxx.webp'），
       不要用 '/' 开头、也不要写 localhost / http:// / file://，
       否则 file:// 双击打开时图片会 404。
     - 资源一律 WebP（保留透明 alpha），不用 PNG。
     - 标题图统一经 js/assetConfig.js 的 ASSET_CONFIG.resolve() 解析（assets/ 前缀）。
   ============================================================ */
...
window.PAW_ASSETS = Object.assign(window.PAW_ASSETS, {
    ...
    // 未来可在此扩展：
    // wallTitle: (__AC && typeof __AC.resolve === 'function') ? __AC.resolve('titles/wall-title.webp') : 'assets/titles/wall-title.webp',
});
